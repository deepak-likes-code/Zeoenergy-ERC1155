...
export default function Home() {
  ...
  async function loadNFTs() {
    ...
    const priceContract = new ethers.Contract(priceFeedAddress, PriceFeed.abi, provider)
            ...
            nfts.map((nft, i) => (
              <div key={i} style={{ border: "grey 1px solid", display: "flex", flexDirection: "column", overflow: "hidden", borderRadius: "20px" }} className="nftItem">
                <div style={{ height: "100%" }}>
                  <embed className="rounded" src={nft.image} />
                </div>
                <div className="text-center px-2 py-3">
                  <p style={{}} className="text-2xl p-3 font-semibold">{nft.name}</p>
                  <p className="text-gray-400">Available : {nft.amount}</p>
                </div>
                <div className="p-4 bg-black " >
                  <p className="text-2xl mb-4 font-bold text-white">{nft.price} ETH</p>
                  <p className="text-2xl mb-4 font-bold text-white">{(nft.price * inUSD).toFixed(5)} USD</p>
                  <input id={i} type="number" min="0" max={nft.amount} placeholder="quantity" className="text-xl mb-4 w-full text-black rounded  p-3 shadow-lg" onChange={e => updateAmountToBuy(e, nft.tokenId)} />
